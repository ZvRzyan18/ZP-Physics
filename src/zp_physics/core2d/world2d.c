...
zp_cold int zp_world2d_init(zp_world2d **const zp_restrict world, const zp_create_world2d *const zp_restrict data) {
	*world = (zp_world2d *)malloc(sizeof(zp_world2d));
	...
	if(zp_unlikely(zp_broadphase2d_init(&world_instance->_broadphase, world_instance->_growth_base, aabb_margin)))
	 return -1;
...
zp_hot void zp_world2d_update(zp_world2d *const zp_restrict world, const float dt) {
 ...
 uint8_t frame_tolerance = 8;
 if(zp_broadphase2d_should_rebuild(&world->_broadphase, frame_tolerance)) {
 ...
 */
	for(uint8_t aa = 0; aa < world->_time_substeps; aa++)	{
		...
		for(size_t i = 0; i < position_integration_size; i++)	{
		 ...
		 zp_aabb2d aabb;
   aabb._min = body_a->_head._aabb[0];
   aabb._max = body_a->_head._aabb[1];

		 zp_broadphase2d_update_element(&world->_broadphase, body_a->_head._aabb_node, aabb);
 	}
...
zp_handle zp_world2d_create_body(zp_world2d *const zp_restrict world, const void *const zp_restrict value){
 ...
 zp_aabb2d aabb;
 aabb._min = body->_head._aabb[0];
 aabb._max = body->_head._aabb[1];
 body->_head._aabb_node = zp_broadphase2d_insert_element(&world->_broadphase, aabb, id);
